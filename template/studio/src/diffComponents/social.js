...
function TextPreviewComponent({ value, diff, from }) {
  ...
  return (
    <div>
      <div>
        <DiffTooltip as="p" diff={diff}>
          {from ? <del>{value}</del> : <span>{value}</span>}
        </DiffTooltip>
      </div>
      {tone ? (
        <div>
          <small>
            Sentiment:{" "}
            {tone.document_tone.tones.reduce(
              (acc, curr, index) =>
                `${acc}${index > 0 ? ";" : ""} ${curr.tone_name} (${(
                  Math.round(curr.score * 100) / 100
                ).toFixed(2)})`,
              ""
            )}
          </small>
